...
<script>
  import { getList, createNav, updateNav, deleteNav } from '@/api/nav'
    ...
    methods: {
      ...
      createData() {
        ...
        if (this.temp.Url == null || this.temp.Name == null) {
          ...
          createNav(this.temp).then(response => {
            if (response.data === 'OK') {
      ...
      handleUpdateStatus() {
        updateNav(this.autoStatusForm).then(response => {
          if (response.data === 'OK') {
      ...
      handleDelete() {
        deleteNav(this.delFormData.Id).then(response => {
          if (response.data === 'OK') {
            this.$notify({
              title: '成功',
              message: '删除成功',
